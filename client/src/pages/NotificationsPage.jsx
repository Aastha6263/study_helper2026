...
const NotificationsPage = () => {
  ...
  const { items: localItems, unreadCount } = useSelector(
    (s) => s.notifications,
  );
  ...
  const displayed =
    filter === 'unread' ? merged.filter((n) => !n.isRead) : merged;
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setServerItems((prev) => prev.filter((n) => n._id !== id && n.id !== id));
    } catch {
